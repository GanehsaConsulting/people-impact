...
export const OurStory = () => {
  const consultingServices = [
    ...
    },
    {
      ...
      description: "Building robust, trust-based relationships with our clients and providing expert guidance and ongoing support to ensure sustained achievement.",
      icon: FaHandshake,
      color: "from-main-1 to-main-1/80",
      bgColor: "bg-main-1/5 dark:bg-main-1/10",
      iconColor: "text-main-1",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-6 md:gap-3 mt-5">
          {consultingServices.map((service, index) => {
            ...
            return (
              <div
                key={service.id}
                className={`group relative overflow-hidden rounded-2xl border transition-all ease-in-out duration-500 hover:shadow-2xl hover:-translate-y-2 text-center p-6 md:p-8 ${
                  isBlue
                    ? "bg-gradient-to-br from-[#119bd3]/5 to-[#119bd3]/10 border-[#119bd3]/20 dark:from-[#119bd3]/10 dark:to-[#119bd3]/20 dark:border-[#119bd3]/30"
                    : "bg-gradient-to-br from-[#8dbb3f]/5 to-[#8dbb3f]/10 border-[#8dbb3f]/20 dark:from-[#8dbb3f]/10 dark:to-[#8dbb3f]/20 dark:border-[#8dbb3f]/30"
                }`}
              >
                {/* Background Gradient Effect */}
                <div className={`absolute inset-0 bg-gradient-to-br opacity-0 group-hover:opacity-10 transition-opacity duration-500 ${isBlue ? "from-[#119bd3]/20 to-[#119bd3]/5" : "from-[#8dbb3f]/20 to-[#8dbb3f]/5"}`}></div>
                ...
                <div className={`relative mb-6 inline-flex items-center justify-center w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-white dark:bg-gray-800 shadow-lg group-hover:shadow-xl transition-all duration-500 group-hover:scale-110`}>
                  <IconComponent className={`w-8 h-8 md:w-10 md:h-10 transition-all duration-500 group-hover:scale-110 ${isBlue ? "text-[#119bd3]" : "text-[#8dbb3f]"}`} />

                  {/* Icon Glow Effect */}
                  <div
                    className={`absolute inset-0 rounded-2xl bg-gradient-to-br opacity-0 group-hover:opacity-20 transition-opacity duration-500 ${isBlue ? "from-[#119bd3]/30 to-[#119bd3]/10" : "from-[#8dbb3f]/30 to-[#8dbb3f]/10"}`}
                  ></div>
                </div>
                ...
                <div className="relative space-y-4">
                  <h3
                    className={`text-xl md:text-2xl font-bold text-gray-900 dark:text-white transition-all duration-500 text-start group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r ${
                      isBlue ? "group-hover:from-[#119bd3] group-hover:to-[#0ea5e9]" : "group-hover:from-[#8dbb3f] group-hover:to-[#84cc16]"
                    }`}
                  >
                    {service.title}
                  </h3>
                ...
                <div
                  className={`absolute bottom-0 left-0 w-16 h-16 bg-gradient-to-tr to-transparent rounded-tr-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${isBlue ? "from-[#119bd3]/10" : "from-[#8dbb3f]/10"}`}
                ></div>
              </div>
            );
          })}
